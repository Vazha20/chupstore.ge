...
export default function ProductsPage() {
  ...
  return (
    <div style={{ maxWidth: "1320px", padding: "40px 30px", margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: "30px", alignItems: "" }}>
        {/* Desktop Sidebar */}
        {windowWidth > 1024 && (
          <div style={{
            minWidth: "300px",
            ...
            <div style={{
              ...
              <h3 style={{ marginBottom: "12px", fontSize: "16px", fontWeight: "700", color: "#111" }}>კატეგორიები</h3>
                  ...
                  <li key={cat}>
                    <button
                      ...
                      style={{
                        ...
                        cursor: "pointer",
                        fontWeight: selectedCategory === cat ? "600" : "600",
                        color: "#333",
                      }}
                    >
                      {cat}
            ...
            {currentProducts.map((p) => (
              <Link key={p.id} href={`/products/${p.id}`} style={{ textDecoration: "none", color: "inherit" }}>
                <div style={{
                  ...
                  <div>
                    {p.image && <img src={p.image} alt={p.name} style={{ width: "100%", height: "180px", objectFit: "fill", borderRadius: "8px", marginBottom: "15px" }} />}
                    <h3 style={{ marginBottom: "10px" }}>{p.name}</h3>
                    <p style={{ marginBottom: "15px", color: "#555" }}>
                      {p.description.length > 60 ? p.description.slice(0, 60) + "..." : p.description}
                    </p>
                  </div>
                  <strong style={{ fontSize: "16px", color: "#000" }}>{p.price} ₾</strong>
                </div>
              </Link>
            ))}
          </div>
